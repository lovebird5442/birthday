...
function initMainSite() {
  ...
  // Initialize draggable papers AFTER main content is visible
  setTimeout(() => {
    const paperEls = document.querySelectorAll(".papers-board .paper");
    if (paperEls.length > 0) {
      paperEls.forEach((p) => {
        // Set initial random positions for each paper
        const randomX = (Math.random() - 0.5) * 150;
        const randomY = (Math.random() - 0.5) * 150;
        const randomRot = Math.random() * 30 - 15;
        p.style.position = "absolute";
        p.style.transform = `translateX(${randomX}px) translateY(${randomY}px) rotateZ(${randomRot}deg)`;
        new DragPaper(p);
      });
    }
...
class DragPaper {
  constructor(el) {
    ...
    // Only set transform if it's not already set
    if (!currentTransform || currentTransform === "none") {
...
// Initialize draggable papers inside initMainSite
// This ensures they're set up after the main content is visible
setTimeout(() => {
  const paperEls = document.querySelectorAll(".papers-board .paper");
  if (paperEls.length > 0) {
    paperEls.forEach((p) => {
      ...
      // Ensure papers are positioned absolutely
